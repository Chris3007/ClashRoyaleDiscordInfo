          ...
          if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            message.channel.send("Your data is being saved...").then((msg) => {
              ...
              pool.getConnection(function(err, connection) {
                ...
                // Use the connection
                connection.query(`SELECT * FROM dcMembers where userid = "${messageId}"`, function(error, rows, fields) {
        ...
        if (!args[1]) {
          message.channel.send("Your data is being collected...").then((msg) => {
            ...
            connection.connect((err) => {
              if (err) {
                msg.edit(`Sorry, something went wrong!`);
                console.log("[" + new Date().toLocaleString() + "] Could not connect to database")
              }
            });
